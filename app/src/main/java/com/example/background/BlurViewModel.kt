...
class BlurViewModel(application: Application) : AndroidViewModel(application) {
    ...
    internal fun applyBlur(blurLevel: Int) {
        ...
        for(i in 0 until blurLevel){
            ...
            if(i == 0){
                blurImageRequest.setInputData(createInputDataFromUri())
            }

            continuation = continuation.then(blurImageRequest.build())
        }
    ...
    private fun createInputDataFromUri(): Data{
        ...
        imageUri?.let { tempUri->
            builder.putString(KEY_IMAGE_URI,tempUri.toString())
        }
